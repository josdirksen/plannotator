...
Replace visual-explainer's `--bg`, `--surface`, `--border`, `--text`, `--accent` variables with Plannotator's semantic tokens. Include these as `:root` defaults so the file works standalone. When rendered in the Plannotator UI (the default for `.html` files), these get overridden by the active theme.
...
Plannotator handles dark/light via theme classes, not `prefers-color-scheme`. The standalone defaults above are the light theme. When rendered in the Plannotator UI (the default for `.html` files), the active theme's tokens override automatically — no media query needed in the generated HTML.
